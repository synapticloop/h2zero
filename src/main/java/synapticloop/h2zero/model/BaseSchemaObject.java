		...
		for(int i = 0; i < finderJson.length(); i++) {
			...
			if(null != autoFinder) {
				if(autoFinder.compareToIgnoreCase(JSONKeyConstants.UNIQUE) == 0) {
					...
					throw new H2ZeroParseException("Found an auto generate finder on '" + this.name + "." + name + "' with a value of '" + autoFinder + "'.  The allowable values are '" + JSONKeyConstants.UNIQUE + "' or '" + JSONKeyConstants.MULTIPLE + "'.");
				}
			}
		}
	}
