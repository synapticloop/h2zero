...
public abstract class BaseField {
	...
	// TODO - this needs to be removed - once they have been deprecated
	private boolean hasDeprecatedForeignKey = false; // whether we have a deprecated 'foreign' key on the json field object
	...
	public BaseField(JSONObject jsonObject) throws H2ZeroParseException {
		...
		try {
			JSONObject foreignKeyObject = jsonObject.getJSONObject(JSONKeyConstants.FOREIGN);

			if(null != foreignKey) {
				throw new H2ZeroParseException("Field '" + name + "' cannot have a '" + JSONKeyConstants.FOREIGN_KEY + "' and a '" + JSONKeyConstants.FOREIGN + "' on the same field.");
			}

			foreignKeyTable = foreignKeyObject.getString("table");
			foreignKeyField = foreignKeyObject.getString("field");
			hasDeprecatedForeignKey = true;
