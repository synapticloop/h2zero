...
//			}
	...
	private void populateFinders(JSONObject jsonObject) throws H2ZeroParseException {
		...
		for (String multipleFinder : autoGeneratedMultipleFinders) {
			...
			try {
				autoFinder.put(Finder.JSON_KEY_NAME, "findBy" + NamingHelper.getFirstUpper(multipleFinder));
				autoFinder.put(Finder.JSON_KEY_WHERE_CLAUSE, "where " + multipleFinder + " = ?");
				autoFinder.put(Finder.JSON_KEY_UNIQUE, false);
				autoFinder.put(Finder.JSON_KEY_WHERE_FIELDS, new JSONArray().put(multipleFinder));
		...
		for (String uniqueFinder : autoGeneratedUniqueFinders) {
			JSONObject autoFinder = new JSONObject();
			try {
				autoFinder.put(Finder.JSON_KEY_NAME, "findBy" + NamingHelper.getFirstUpper(uniqueFinder));
				autoFinder.put(Finder.JSON_KEY_WHERE_CLAUSE, "where " + uniqueFinder + " = ?");
				autoFinder.put(Finder.JSON_KEY_UNIQUE, true);
				autoFinder.put(Finder.JSON_KEY_WHERE_FIELDS, new JSONArray().put(uniqueFinder));
