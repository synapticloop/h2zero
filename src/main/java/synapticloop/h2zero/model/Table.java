...
public class Table {
	...
	private void populateFields(JSONObject jsonObject) throws H2ZeroParseException {
		...
		try {
			fieldJson = jsonObject.getJSONArray(JSONKeyConstants.JSON_KEY_FIELDS);
		} catch (JSONException ojjsonex) {
			throw new H2ZeroParseException("Cannot create a table without '" + JSONKeyConstants.JSON_KEY_FIELDS + "'.");
		}
		...
		for (int i = 0; i < fieldJson.length(); i++) {
			...
			try {
				...
				name = fieldObject.getString(JSONKeyConstants.JSON_KEY_NAME);
				if(null == name) {
	...
	private void populateFieldFinders(JSONObject jsonObject) throws H2ZeroParseException {
		...
		try {
			finderJson = jsonObject.getJSONArray(JSONKeyConstants.JSON_KEY_FIELD_FINDERS);
		} catch (JSONException ojjsonex) {
		...
		for(int i = 0; i < finderJson.length(); i++) {
			...
			if(null == fieldFinderObject) {
				throw new H2ZeroParseException("Found a '" + JSONKeyConstants.JSON_KEY_FIELD_FINDERS +  "' json array on table '" + this.name + "', however the value at index '" + i + "' is not a json object.");
			}
