...
import com.synapticloop.h2zero.util.KeyHelper;
...
public class Table extends BaseSchemaObject {
	...
	private boolean hasExternalForeignKey = false;
	...
	// this list is for foreign keys on this table that point to other tables
	private final List<BaseField> foreignKeys = new ArrayList<>();
	// this list is for foreign keys from other tables that point to this table
	// used for documentation and testing whether the table can be truncated
	// it will be in the format of <table_name>.<field_name>
	private final List<String> externalForeignKeys = new ArrayList<>();
	...
	private final List<Updater> updaters = new ArrayList<>(); // a list of all updaters
	private final List<Inserter> inserters = new ArrayList<>(); // a list of all inserters
	private final List<Upserter> upserters = new ArrayList<>(); // a list of all upserters
	private final List<Deleter> deleters = new ArrayList<>(); // a list of all deleters
	private final List<Constant> constants = new ArrayList<>(); // a list of all constants
	private final List<ConstantCache> constantCaches = new ArrayList<>(); // a list of all constant caches

	protected List<String> autoGeneratedDeleters = new ArrayList<>(); // a list of all automatically generated deleters
	...
	public Table(Options options, JSONObject jsonObject, int defaultStatementCacheSize) throws H2ZeroParseException {
		...
		FieldLookupHelper.addToTableCache(this.name, this);
	}

	/**
	 * <p>Populate all actions that can be performed on this table, including</p>
	 *
	 * <ul>
	 *   <li>field finders</li>
	...
	private void populateFields(JSONObject jsonObject) throws H2ZeroParseException {
		...
		for (BaseField baseField : fields) {
			...
			if (null != foreignKeyField && null != foreignKeyTable) {
				...
				// now we need to reach into the other table and place an external
				// foreign field on it - helps with the delet vs. truncate logic
				Table table = FieldLookupHelper.getFromTableCache(foreignKeyTable);
				if(null == table) {
					// unlikely as we do so many checks beforehand
					throw new H2ZeroParseException(String.format("Field '%s' on table '%s' is trying to reference a foreign key of '%s.%s' which has not been defined yet.", baseField.getName(), name, foreignKeyTable, foreignKeyField));
				} else {
					table.addExternalForeignKey(table.getName(), baseField.getName());
				}
			}
		}
	...
	public boolean getHasExternalForeignKey() {
		return(externalForeignKeys.size() != 0);
	}

	public List<String> getExternalForeignKeys() {
		return (externalForeignKeys);
	}

	public void addExternalForeignKey(String foreignKeyTableName, String foreignKeyFieldName) {
		externalForeignKeys.add(foreignKeyTableName + "." + foreignKeyFieldName);
	}
