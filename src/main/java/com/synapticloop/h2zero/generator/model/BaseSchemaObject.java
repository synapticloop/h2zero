...
public abstract class BaseSchemaObject {
	...
	private final List<Finder> finders = new ArrayList<>(); // a list of all finders
	private final List<Counter> counters = new ArrayList<>(); // a list of all counters
	private final List<Question> questions = new ArrayList<>(); // a list of all questions

	protected final List<String> autoGeneratedUniqueFinders = new ArrayList<>(); // a list of all automatically generated 'unique' result finders
	protected final List<String> autoGeneratedMultipleFinders = new ArrayList<>(); // a list of all automatically generated 'multiple' result finders

	protected final List<String> autoGeneratedUniqueNullFinders = new ArrayList<>(); // a list of all automatically generated 'unique' result finders
	protected final List<String> autoGeneratedMultipleNullFinders = new ArrayList<>(); // a list of all automatically generated 'multiple' result finders

	protected final List<String> autoGeneratedUniqueNotNullFinders = new ArrayList<>(); // a list of all automatically
	protected final List<String> autoGeneratedMultipleNotNullFinders = new ArrayList<>(); // a list of all automatically

	protected final List<JSONObject> autoGeneratedOrderedFinders = new ArrayList<>(); // a list of all automatically generated 'unique' result finders
	protected final List<String> autoGeneratedFindAllOrdered = new ArrayList<>(); // a list of all automatically
	// generated 'unique' result finders

	protected final List<String> autoGeneratedUpdaters = new ArrayList<>(); // a list of all automatically generated updaters from the 'fieldUpdaters' JSON key
	protected final List<String> autoGeneratedCounters = new ArrayList<>(); // a list of all automatically generated counters from the 'fieldCounters' JSON key

	protected Map<String, BaseField> fieldLookup = new HashMap<>(); // a quick lookup map of all fields for this table
	protected Map<String, BaseField> inFieldLookup = new HashMap<>(); // a quick lookup for all 'in' fields

	protected List<BaseField> fields = new ArrayList<>();  // a list of all fields on this table
	protected final List<BaseField> uniqueFields = new ArrayList<>();

	protected List<BaseField> populateFields = new ArrayList<>();  // a list of all fields on this table
	protected List<BaseField> nonPopulateFields = new ArrayList<>();  // a list of all fields on this table

	protected BaseField primaryKeyField = null; // the field that is the primary key

	protected Set<String> referencedFieldTypes = new HashSet<>(); // this is a set of all referenced field types
	...
	/**
	 * <p>Populate the find all ordered</a>
	 *
	 * @param jsonObject The JSON object to lookup
	 */
	protected void populateFindAllOrdered(JSONObject jsonObject) throws H2ZeroParseException {
		JSONArray findAllOrderedJsonArray;
		try {
			findAllOrderedJsonArray = jsonObject.getJSONArray(JSONKeyConstants.FIND_ALL_ORDERED);
		} catch (JSONException ignored) {
			// no ordering of the findAll is fine
			return;
		}

		// at this point we have some find all ordering
		// go through and parse them
		for(int i = 0; i < findAllOrderedJsonArray.length(); i++) {
			String findAllOrderedStatement = findAllOrderedJsonArray.optString(i, null);
			if (null == findAllOrderedStatement) {
				throw new H2ZeroParseException("Found a '" +
						JSONKeyConstants.FIND_ALL_ORDERED +
						"' json array on table/view '" +
						this.name +
						"', however the value at index '" +
						i +
						"' is not a string.");
			}

			// at this point we have the orderby
			autoGeneratedFindAllOrdered.add(findAllOrderedStatement);
		}

		jsonObject.remove(JSONKeyConstants.FIND_ALL_ORDERED);
	}
	...
	protected void populateFieldFinders(JSONObject jsonObject) throws H2ZeroParseException {
		...
		JSONArray fieldFindersJson = new JSONArray();
		try {
			fieldFindersJson = jsonObject.getJSONArray(JSONKeyConstants.FIELD_FINDERS);
		} catch (JSONException ojjsonex) {
			// do nothing - no field finders is ok
			return;
		}

		for(int i = 0; i < fieldFindersJson.length(); i++) {
			JSONObject fieldFinderObject = fieldFindersJson.optJSONObject(i);
			...
			// at this point you may have one of two field finders

			// {
			//    "fields":  "num_strata_plan,fl_is_current",
			//    "orderBy":  "num_strata_plan asc",
			//    "unique": false
			//    },

			// OR

			// { "num_strata_plan": "multiple" }

			String orderFields = fieldFinderObject.optString(JSONKeyConstants.FIELDS, null);
			if(null != orderFields) {
				// we have a field finder which allows order by
				// we need two more fields:
				// unique: true/false
				// orderBy

				String orderBy = fieldFinderObject.optString(JSONKeyConstants.ORDER_BY, null);
				Boolean unique = fieldFinderObject.optBooleanObject(JSONKeyConstants.UNIQUE, null);
				if(null == orderBy && null == unique) {
					throw new H2ZeroParseException("Found an auto generate ordered finder on '" +
							this.name + " with fields " +
							orderFields +
							"' without both an 'orderBy' and 'unique' key.");
				} else if(null == orderBy) {
					throw new H2ZeroParseException("Found an auto generate ordered finder on '" +
							this.name + " with fields " +
							orderFields +
							"' without an 'orderBy' field");
				} else if(null == unique) {
					throw new H2ZeroParseException("Found an auto generate ordered finder on '" +
							this.name + " with fields " +
							orderFields +
							"' without a 'unique' field");
				}

				// at this point we are good to go
				autoGeneratedOrderedFinders.add(fieldFinderObject);
			} else {
				// normal field finder
	...
	protected void populateFieldNotNullFinders(JSONObject jsonObject) throws H2ZeroParseException {
		// now for the auto-generated finders
		JSONArray finderJson = new JSONArray();
		try {
			finderJson = jsonObject.getJSONArray(JSONKeyConstants.FIELD_NOT_NULL_FINDERS);
		} catch (JSONException ignored) {
			// do nothing - no finders is ok
			return;
		}
		...
		for(int i = 0; i < finderJson.length(); i++) {
			...
			if(null == fieldFinderObject) {
				throw new H2ZeroParseException("Found a '" + JSONKeyConstants.FIELD_NOT_NULL_FINDERS +  "' json array on table/view '" + this.name + "', however the value at index '" + i + "' is not a json object.");
			}

			Iterator<String> keys = fieldFinderObject.keys();
			// should only be one key
			String autoFinderName = keys.next();
			String autoFinder = fieldFinderObject.optString(autoFinderName);

			if(null != autoFinder) {
				if(autoFinder.compareToIgnoreCase(JSONKeyConstants.UNIQUE) == 0 || autoFinder.compareToIgnoreCase(JSONKeyConstants.SINGLE) == 0) {
					autoGeneratedUniqueNotNullFinders.add(autoFinderName);
				} else if(autoFinder.compareToIgnoreCase(JSONKeyConstants.MULTIPLE) == 0) {
					autoGeneratedMultipleNotNullFinders.add(autoFinderName);
				} else {
					throw new H2ZeroParseException("Found an auto generate finder on '" + this.name + "." + autoFinderName + "' with a value of '" + autoFinder + "'.  The allowable values are '" + JSONKeyConstants.UNIQUE + "', '" + JSONKeyConstants.SINGLE + "' or '" + JSONKeyConstants.MULTIPLE + "'.");
				}
			}
		...
		jsonObject.remove(JSONKeyConstants.FIELD_NOT_NULL_FINDERS);
	}
	...
	protected void populateFinders(JSONObject jsonObject) throws H2ZeroParseException {
		...
		// findAllFinders
		for (String findAllOrdered : autoGeneratedFindAllOrdered) {
			generateAutomaticFindAllOrderedFinder(findAllOrdered);
		}
		...
		for (JSONObject orderedFinder : autoGeneratedOrderedFinders) {
			generateAutomaticFullFieldFinder(orderedFinder);
		}
		...
		// now go through and addNullFinders based on the multiple and unique finders
		for (String multipleFinder : autoGeneratedMultipleNotNullFinders) {
			generateAutomaticNotNullFinder(multipleFinder, false);
		}

		for (String uniqueFinder : autoGeneratedUniqueNotNullFinders) {
			generateAutomaticNotNullFinder(uniqueFinder, true);
		}
	...
	private void generateAutomaticFindAllOrderedFinder(String findAllOrdered) throws H2ZeroParseException {
		JSONObject autoFinder = new JSONObject();

		String[] orderByField = findAllOrdered.split(",");

		StringBuilder finderNameBuilder = new StringBuilder();
		finderNameBuilder.append("findAllOrderedBy");
		...
		try {
			for (int i = 0; i < orderByField.length; i++) {
				...
				String orderByAscDesc = orderByField[i].trim();
				String[] orderByNameValue = orderByAscDesc.split(" ");
				if (orderByNameValue.length != 2) {
					throw new H2ZeroParseException("There is an ordered field finder which is not in the proper format, it must" +
							" include an 'asc' or 'desc'");
				}

				if (!orderByNameValue[1].equalsIgnoreCase("asc") && !orderByNameValue[1].equalsIgnoreCase("desc")) {
					throw new H2ZeroParseException("There is an ordered field finder which is not in the proper format, it must" +
							" include an 'asc' or 'desc', found '" + orderByNameValue[1] + "'.");
				}

				finderNameBuilder.append(NamingHelper.getFirstUpper(orderByNameValue[0]));
				finderNameBuilder.append(NamingHelper.getFirstUpper(orderByNameValue[1]));
				orderByClauseBuilder.append(orderByAscDesc);
			}

			autoFinder.put(JSONKeyConstants.NAME, finderNameBuilder.toString());
			autoFinder.put(JSONKeyConstants.ORDER_BY, orderByClauseBuilder.toString());

			autoFinder.put(JSONKeyConstants.UNIQUE, false);

			Finder finder = new Finder(this, autoFinder);
			finder.setIsAutoFinder(true);
			finder.setIsFullAutoFinder(true);
			finders.add(finder);
		} catch (JSONException jsonex) {
			throw new H2ZeroParseException("Could not generate the find all ordered finder for '" + findAllOrdered + "'.",
					jsonex);
		}
	}
	...
	/**
	 * <p>This generates the full definition finder with all available options.</p>
	 *
	 * <p>The options are as follows:</p>
	 *
	 * <ul>
	 *   <li><code>name</code> [OPTIONAL] the name of the finder</li>
	 *   <li><code>fields</code> [MANDATORY] the field definitions</li>
	 *   <li><code>orderBy</code> [OPTIONAL] The order by phrases</li>
	 *   <li><code>unique</code> [MANDATORY] must be one of <code>true</code> or <code>false</code></li>
	 * </ul>
	 *
	 * @param finderObject The finder JSON object to parse into the finder
	 *
	 * @throws H2ZeroParseException If there was an error with the parsing of the document
	 */
	private void generateAutomaticFullFieldFinder(JSONObject finderObject) throws H2ZeroParseException {
		String[] fields = finderObject.getString(JSONKeyConstants.FIELDS).split(",");
		JSONArray whereFieldsArray = new JSONArray();

		try {
			StringBuilder finderNameBuilder = new StringBuilder();
			finderNameBuilder.append("findBy");

			StringBuilder whereClauseBuilder = new StringBuilder();
			whereClauseBuilder.append("where ");

			// the fields that we are going to parse will be of the format:
			// 'null'|'notNull' : <field_name>
			// if it starts with 'null' or 'notNull' then we will set the where clause
			//   appropriately
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if(field.toLowerCase().startsWith("null:")) {
					isNullable = true;
					field = field.substring("null:".length());
				}

				if(field.toLowerCase().startsWith("notnull:")) {
					isNot = true;
					isNullable = true;
					field = field.substring("notnull:".length());
				}

				if(field.contains(":")) {
					throw new H2ZeroParseException("The Full Field Finder of '" +
							finderObject.toString() +
							"' has a 'fields' definition with an unknown nullability prefix.");
				}

				finderNameBuilder.append(NamingHelper.getFirstUpper(field));
				...
				whereClauseBuilder.append(field);
				if(isNullable) {
					whereClauseBuilder.append(" is ");
					if(isNot) {
						whereClauseBuilder.append(" not ");
						finderNameBuilder.append("Not");
					}
					whereClauseBuilder.append(" null ");
					finderNameBuilder.append("Null");
				} else {
					whereClauseBuilder.append(" = ?");
					whereFieldsArray.put(field);
				}
			...
			// now the orderBy
			String[] orderByField = finderObject.getString(JSONKeyConstants.ORDER_BY).split(",");

			finderNameBuilder.append("OrderedBy");
			...
			// if the finder has a name, use the name otherwise use the generated one
			autoFinder.put(JSONKeyConstants.NAME,
					finderObject.optString(JSONKeyConstants.NAME,
							finderNameBuilder.toString()));

			autoFinder.put(JSONKeyConstants.ORDER_BY, orderByClauseBuilder.toString());

			autoFinder.put(JSONKeyConstants.WHERE_CLAUSE, whereClauseBuilder.toString());
			autoFinder.put(JSONKeyConstants.UNIQUE, finderObject.getBoolean(JSONKeyConstants.UNIQUE));
			autoFinder.put(JSONKeyConstants.WHERE_FIELDS, whereFieldsArray);
			...
			throw new H2ZeroParseException("Could not generate the ordered field finder for '" +
					finderObject.toString() +
					"'.",
					jsonex);
		}
	 ...
	 * @throws H2ZeroParseException If something went horribly wrong
	 */
	private void generateAutomaticNotNullFinder(String uniqueFinder, boolean unique) throws H2ZeroParseException {
		JSONObject autoFinder = new JSONObject();
		String[] fields = uniqueFinder.split(",");
		JSONArray whereFieldsArray = new JSONArray();
		...
		try {
			StringBuilder fieldNameBuilder = new StringBuilder();
			fieldNameBuilder.append(JSONKeyConstants.FIND_BY_NOT_NULL);
			StringBuilder whereClauseBuilder = new StringBuilder();
			whereClauseBuilder.append("where ");
			...
			for (int i = 0; i < fields.length; i++) {
				...
				whereClauseBuilder.append(field);
				whereClauseBuilder.append(" is not null");
	...
	/**
	 * Populate all questions that are being generated for a table.  A question is a simple query that returns one
