...
public class AuthorStatusImporter {
...
// txt_desc_author_status
	 ...
	 * @param txtAuthorStatus The txt_author_status which is used as the lookup
	 * @param txtDescAuthorStatus The txt_desc_author_status which is used as the lookup
	 * 
	 ...
	 */
	private static boolean confirmExisting(Long idAuthorStatus, String txtAuthorStatus, String txtDescAuthorStatus) throws H2ZeroParseException {
		StringBuilder SQL_FIND_EXACT = new StringBuilder("select * from author_status where ");
		if (null == txtAuthorStatus) {
			SQL_FIND_EXACT.append(" txt_author_status is null");
		} else {
			SQL_FIND_EXACT.append(" txt_author_status = ?");
		}
		SQL_FIND_EXACT.append(" and ");
		if (null == txtDescAuthorStatus) {
			SQL_FIND_EXACT.append(" txt_desc_author_status is null");
		} else {
			SQL_FIND_EXACT.append(" txt_desc_author_status = ?");
		}
		...
		throw new H2ZeroParseException("Could not find a constant table with values [" + idAuthorStatus + ", " + txtAuthorStatus + ", " + txtDescAuthorStatus + "] a code change will be required.");
	}
